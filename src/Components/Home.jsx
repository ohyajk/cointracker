  ...
  return (
    <div data-testid="home-1" className="">
      ...
      <div>
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 clr">
          {allCoins?.filter((coin) => (coin.name.toLowerCase().includes(query)))
            .map((coinf) => (
              <Link to="/coin" onClick={() => { dispatch(addCoin(coinf)); }} key={coinf.id}>
                <li className="flex items-center py-2 px-4 m-2 gap-2 bg-violet-900/20 border border-black/10">
